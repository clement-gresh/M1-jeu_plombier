        ...
        for(Dir[] directions : ouvertures[this.ordinal()]){
            for(int i = 0; i < directions.length; i++){
                if(directions[i] == dirModele) {
                    // On enleve l'ouverture d'entree et on applique la rotation
                    // aux ouvertures restantes
                    ArrayList<Dir> dirConnectees = 
                            new ArrayList<>(Arrays.asList(directions));
                    dirConnectees.remove(i);
                    
                    for(int j = 0; j < dirConnectees.size(); j++){ 
                        dirConnectees.set(j,
                                dirConnectees.get(j).rotation(rotation, AJOUT));
                    }
